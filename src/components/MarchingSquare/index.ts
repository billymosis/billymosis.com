...
class Main {
  pane = new Pane({
    container: document.getElementById('tweak-pane') ?? undefined,
  });
  app = new Application();
  ...
  async init() {
    ...
    await this.app.init({ resizeTo: el, backgroundAlpha: 0 });
    window.onresize = () => {
      this.app.resize();
      this.setupGrid({});
    };
    ...
    this.app.stage.on('touchendcapture', (e) => {
      console.log('touch');
      this.pointer.x = e.x;
      this.pointer.y = e.y;
    });
    ...
    this.setupGrid({});
    Array.from({ length: window.innerWidth <= 768 ? 4 : 8 }, () => {
      this.circles.push(
        new Circle({
  ...
  setupPane() {
    ...
    this.pane.addBinding(PARAMS, 'color').on('change', ({ value }) => {
      for (let i = 0; i < this.rects.length; i++) {
        this.rects[i].changeColor(value);
      }
    });
    this.pane
